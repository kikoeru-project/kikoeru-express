...
const insertWorkMetadata = work => knex.transaction(
    trx => trx
        .raw(
            trx('t_circle')
                .insert({
 ...
 */
const cleanupOrphans = async (trxProvider, circle, series, tags, vas) => {
    ...
    promises.push(new Promise((resolveSeries, rejectSeries) => {
        getCount('t_work', 'series_id', series)
            .then((count) => {
                if (count === 0) {
                    trx('t_series')
                        .del()
...
/**
 * Removes a work and then its orphaned circles, tags series & VAs from the database.
 * @param {Integer} id Work id.
 */
const removeWork = async (id, trxProvider) => {
    const trx = await trxProvider();
    // Save circle, tags series and VAs to array for later testing
    const circle = await trx('t_work').select('circle_id').where('id', '=', id).first();
    const series = await trx('t_work').select('series_id').where('id', '=', id).first();
...
/**
 * Returns list of works by circle, tag or VA or series.
...
const getWorksByKeyWord = ({keyword, username = 'admin'} = {}) => {
    ...
    const workIdQuery = knex('r_tag_work').select('work_id').where('tag_id', 'in', tagIdQuery)
...
/**
 * 获取所有社团/标签/声优的元数据列表
 * @param {Starting} field ['circle', 'tag', 'va' ,'series'] 中的一个
 */
...
const getMetadata = ({field = 'circle', id} = {}) => {
    const validFields = ['circle', 'series', 'tag', 'va'];
